...
export default function Sidebar({ open, isDesktop, page, setPage, profile, onClose }) {
  ...
  const inner = (
    ...
    >
      ...
      <div style={{
        ...
        {showLabels && (
          <div style={{ minWidth: 0, flex: 1 }}>
            <p style={{ fontWeight: 700, color: "var(--text-primary)", fontSize: 13, lineHeight: 1, letterSpacing: "-0.02em" }}>
              SkillGap AI
            </p>
        ...
        {!isDesktop && (
          <button onClick={onClose} aria-label="Close menu"
            style={{
              width: 28, height: 28, borderRadius: 8, marginLeft: "auto",
      ...
      {/* Main nav — flex:1 ensures it takes all available space */}
      <nav
        role="navigation"
        ...
        style={{
          ...
          flexDirection: "column",
          gap: 2,
          /* Prevent gap at bottom by not using justify-content: space-between */
        }}
      >
        {showLabels && (
          <p style={{
        ...
        {mainNav.map((item) => (
          <NavItem
            ...
            onClick={() => { setPage(item.id); if (!isDesktop) onClose?.(); }}
          />
        ))}
      </nav>

      {/* Bottom nav — pinned to bottom, no gap */}
      <div style={{
        ...
        {showLabels && (
          <button
            onClick={() => { setPage("profile"); if (!isDesktop) onClose?.(); }}
...
function NavItem({ item, active, showLabel, onClick }) {
  ...
  return (
    ...
    >
      <Icon size={17} style={{ flexShrink: 0, color: active ? "#818cf8" : "var(--text-muted)" }} />
      {showLabel && (
        <span style={{
          fontSize: 13,
